...
class SimCLRStage1(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.g(feature)  # 得到(batch_size, feature_dim)
...
class Loss(torch.nn.Module):
    ...
    def forward(self, out_1, out_2, batch_size, temperature=0.1):
...
if __name__ == "__main__":
    ...
    f = nn.Sequential(*f)
    print(f)
    g = nn.Sequential(nn.Linear(2048, 512, bias=False),
                      nn.BatchNorm1d(512),
                      nn.ReLU(inplace=True),
                      nn.Linear(512, 128, bias=True))
    input = torch.randn(7, 3, 256, 256)
    output = f(input)
